...
export function legacyRepoDbPath(repoRoot) {
    return join(grapheneDir(repoRoot), "context.db");
}
export function legacyGlobalDbPath() {
...
// back to "node".
function normalizeLegacyName(raw) {
...
function dedupe(candidate, taken) {
...
function buildNodeRenames(rows) {
    const taken = new Set();
    const finalNameOf = new Map();
    const renamedRaw = new Set();
    const renamed = [];
...
function resolveNodeName(raw, renames) {
...
function migrateNodes(db) {
    const nodeRows = db
        .prepare("SELECT name, type, summary, entry_points, covers, last_commit, metadata FROM nodes ORDER BY name ASC")
        .all();
    const edgeRows = db
        .prepare("SELECT from_node, to_node, type, reason FROM edges ORDER BY rowid ASC")
        .all();
    const obsRows = db
        .prepare("SELECT id, node_name, content, source, created_at FROM observations ORDER BY created_at ASC, id ASC")
        .all();
    ...
    for (const o of obsRows) {
        ...
        if (finalName === undefined)
            continue; // orphaned observation: no node to attach it to
    ...
    const nodes = nodeRows.map((row) => {
        ...
        const observations = (obsByNode.get(finalName) ?? []).map((o) => {
...
function migrateFacts(rows) {
    ...
    for (const row of rows) {
        ...
        if (takenPairs.has(`${category}__${subject}`)) {
            let n = 2;
            while (takenPairs.has(`${category}__${baseSubject}-${n}`))
                n++;
...
function rewriteGitignore(repoRoot) {
    ...
    const rewritten = lines.map((line) => {
        ...
        if (trimmed !== ".graphene/" && trimmed !== ".graphene")
            return line;
        changed = true;
...
export function migrateRepo(repoRoot) {
    ...
    for (const fact of facts)
        writeFact(factsDir(repoRoot), fact);
...
export function migrateGlobal(globalDirPath) {

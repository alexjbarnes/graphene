...
export function migrateRepo(db, repoId, repoRoot) {
    ...
    try {
        const importAll = db.transaction(() => {
            if (tableExists(legacy, "nodes")) {
                const rows = legacy
                    .prepare("SELECT name, type, summary, entry_points, covers, last_commit, metadata, created_at, updated_at FROM nodes")
                    .all();
                const ins = db.prepare(`INSERT OR IGNORE INTO nodes
           (repo_id, name, type, summary, entry_points, covers, last_commit, metadata, created_at, updated_at)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`);
                for (const r of rows) {
                    ins.run(repoId, r.name, r.type, r.summary, r.entry_points ?? "[]", r.covers ?? "[]", r.last_commit, r.metadata ?? "{}", r.created_at, r.updated_at);
                }
            }
            if (tableExists(legacy, "edges")) {
                const rows = legacy
                    ...
                    .all();
                const ins = db.prepare(`INSERT OR IGNORE INTO edges (repo_id, from_node, to_node, type, reason, created_at)
           VALUES (?, ?, ?, ?, ?, ?)`);
                for (const r of rows) {
            ...
            if (tableExists(legacy, "observations")) {
                const rows = legacy
                    ...
                    .all();
                const ins = db.prepare(`INSERT INTO observations (repo_id, node_name, content, source, created_at)
           VALUES (?, ?, ?, ?, ?)`);
                for (const r of rows) {
            ...
            if (tableExists(legacy, "project_facts")) {
                const rows = legacy
                    .prepare("SELECT category, subject, content, created_at, updated_at FROM project_facts")
                    .all();
                const ins = db.prepare(`INSERT OR IGNORE INTO project_facts (repo_id, category, subject, content, created_at, updated_at)
           VALUES (?, ?, ?, ?, ?, ?)`);
                for (const r of rows) {
...
export function migrateGlobal(db) {
    ...
    try {
        if (tableExists(legacy, "facts")) {
            const rows = legacy
                ...
                .all();
            const ins = db.prepare(`INSERT OR IGNORE INTO facts (category, subject, content, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?)`);
            const importAll = db.transaction(() => {
                for (const r of rows) {

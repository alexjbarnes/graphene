...
function scanForRepos(dir, depth, cwd, out) {
    let entries;
    try {
...
export function discoverScopes(cwd) {
    let repoRoot;
    try {
    ...
    if (found.length > MAX_SCOPES) {
        throw new Error(`Found ${found.length} repos under ${cwd}, more than the cap of ${MAX_SCOPES}. ` +
...
export function parseNodeRef(ref, scopes) {
...
export function resolveNodeRef(ref, scopes) {
    ...
    if (hits.length === 1)
        return { scope: hits[0], name };
    if (hits.length > 1) {
        throw new Error(`Ambiguous node "${name}": found in ${hits.map((s) => `${s.name}:${name}`).join(", ")}. ` +
...
export function resolveUpsertRef(ref, scopes) {
    const parsed = parseNodeRef(ref, scopes);
    if ("scope" in parsed)
        return parsed;
    const name = parsed.name;
...
export function resolveWriteTarget(scopes, name, cwd, coverPaths) {
    ...
    for (const p of coverPaths) {
        ...
        if (byCwd.length > 1)
            continue; // structurally shouldn't happen: scope roots never nest
    ...
    if (votes.size === 1)
        return [...votes][0];
    throw new Error(`Cannot determine the owning repo for new node "${name}". In scope: ${scopeNames(scopes)}. ` +
...
// through unchanged.
export function rewriteRepoRelative(scope, cwd, paths) {
    return paths.map((p) => {
        const cwdRelCandidate = resolve(cwd, p);
